...
const designerHeader = [
    ...
    // Example: add more items to test mobile pagination
...
export const Designers = () => {
    ...
    const [imgData, setImgData] = useState(null); // Default to null
    const [data, setData] = useState([]);
    const [startIndex, setStartIndex] = useState(0);
    const [lastIndex, setLastIndex] = useState(2); // Shows items at index 0, 1, 2
    const [isMobile, setIsMobile] = useState(window.innerWidth <= 500);

    // Consistent items per view for mobile
    const itemsPerMobileView = 3;
    ...
    const handleModel = (selectedImgData) => {
        setImgData(selectedImgData);
        setOpen(true);
    };
    const handleClose = () => {
        setOpen(false);
        setImgData(null); // Clear imgData when model closes
    };
        ...
        <div className='designers' >
            ...
            <div className='designers-content'>
                <div className='designers-content-list'>
                    {data?.length > 0 ? (
                        data.map((ele) => ( // Use ele.id if available and unique
                            <div className='designer-parent-content' key={ele.id || ele.name}> {/* ADDED KEY */}
                                <div className='designers-content-img' onClick={() => handleModel(ele)}>
                                    <img
                                        src={`${process.env.REACT_APP_BASE_URL}uploads/${ele?.image}`}
                                        alt={ele?.name || 'Designer artwork'} // ADDED ALT
                                    />
                                </div>
                        ...
                        <div className='no-found'>No Data Found for {category}</div>
                    )}
                </div>
            </div>
