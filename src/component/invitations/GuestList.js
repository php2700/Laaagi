...
//   const [totalbox, setTotalBox] = useState();
//   const [totalGuest, setTotalGuest] = useState()
//   // FIX 2: Initialize userBox with an empty string instead of 1
...
//   const [guest, setGuest] = useState([])
//   const [userData, setUserData] = useState({})
//   // FIX 1: Removed the unnecessary localInputs state
...
//         // ---> 2. यह लाइन जोड़ें: boxes स्टेट से भी हटाएं <---
//         setBoxes(boxes.filter((ele) => ele.idx !== index));

//         // 3. guest स्टेट से हटाएं (यह भी पहले से सही है)
//         setGuest(guest.filter((item) => item.idx !== index));

//     } else {
//         // जोड़ने वाला लॉजिक (इसमें कोई बदलाव नहीं)
//         const existingBox = boxes.find((b) => b.idx === index);
...
//             setBoxes(
//                 boxes.map((ele) =>
//                     ele.idx === index ? { ...ele, quantity: quantity } : ele
//                 )
//             );
//         } else {
//             setBoxes([...boxes, { idx: index, quantity: quantity }]);
//         }
...
//                 <td className='category-guest'>{guest.category}</td>
//                 <td className='guest-quantity'>
//                   {/* FIX 1: Bind value directly to the 'boxes' state */}
//                   <input
//                     type='text'
//                     className='invite-guest-list'
//                     value={boxes.find((ele) => ele.idx === index)?.quantity || ''}
//                     onChange={(e) => {
//                       const value = e.target.value;
//                       if (/^\d*$/.test(value)) {
//                         handleBox(value === "" ? 0 : Number(value), index);
